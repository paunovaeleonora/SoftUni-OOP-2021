...
class Television(PowerOutletor, RCAConnector, HDMConnector):
...
class DVDPlayer(HDMConnector, PowerOutletor):
...
class GameConsole(HDMConnector, EthernetConnector, PowerOutletor):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
